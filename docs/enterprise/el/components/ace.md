...
```bash
sudo dnf install pgedge-ace
```

The following table describes the files and directories the package creates:
...
Run the following commands to generate the default configuration templates:
...
Once the configuration files are in place, use the following steps to enable
and start the ACE service.
...
All ACE service logs are written to `/var/log/pgedge/ace/ace.log`. Use the
following command to follow the logs in real time:
...
Run a table diff against your cluster to confirm ACE can connect to all nodes:
...
```bash
sudo tail -50 /var/lib/pgsql/17/data/log/$(ls -t /var/lib/pgsql/17/data/log/ | head -1)
```
...
The following resources provide additional information about installing and
using ACE:
