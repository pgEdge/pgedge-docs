...
pgEdge Distributed Postgres (Cloud Edition) uses an `App Registration` to
provision clusters in a specific `Resource Group` within your Azure
subscription. You can create an `App Registration` and an associated `Client
Secret` in the Azure console under `Microsoft Entra ID` --> `App
Registrations`.

You can use a new or existing `Resource Group`. The following roles must be
assigned to the `App Registration` on the `Resource Group` prior to creating
a cluster:
...
* Provide your Tenant ID in the `Azure Tenant ID` field. To find your tenant
  ID in the Azure console, go to `Microsoft Entra ID` --> `Properties` -->
  `Directory ID`.
* Provide your Resource Group in the `Azure Resource Group` field.

After providing connection information, select `Link Account` to create a
link to your Azure account.
